...
// Universal Fail-Safe AutoTable Invoker
        ...
        function handleCustomWALogoUpload(event) {
            const file = event.target.files[0];
            if (file) {
                const reader = new FileReader();
                reader.onload = function(e) {
                    appState.customWALogo = e.target.result;
                    saveState();
                    showToast('✅ Custom WhatsApp Logo uploaded & saved!');
                };
                reader.readAsDataURL(file);
            }
        }

        function handleCustomGmailLogoUpload(event) {
            const file = event.target.files[0];
            if (file) {
                const reader = new FileReader();
                reader.onload = function(e) {
                    appState.customGmailLogo = e.target.result;
                    saveState();
                    showToast('✅ Custom Gmail Logo uploaded & saved!');
                };
                reader.readAsDataURL(file);
            }
        }
        ...
        function generateMasterPathLabPDFReport(patient, reportObj) {
            try {
                ...
                // --- TOP RIGHT CONTACT DETAILS WITH REAL EMBEDDED WHATSAPP & GMAIL LOGOS ---
                ...
                const logoSize = 5.5; // mm
                const waLogoX = 196 - phoneWidth - logoSize - 1.2; // 1.2mm gap

                const waLogoToDraw = (appState && appState.customWALogo) || (typeof USER_WA_BLACK_B64 !== 'undefined' ? USER_WA_BLACK_B64 : WA_LOGO_B64);

                if (waLogoToDraw) {
                    doc.addImage(waLogoToDraw, 'PNG', waLogoX, 12.5, logoSize, logoSize);
                }

                doc.text(phoneStr, 196, 17, { align: 'right' });
                ...
                const gmailLogoWidth = 5.5;
                const gmailLogoX = 196 - emailWidth - gmailLogoWidth - 1.2; // 1.2mm gap
                ...
                if (gmailLogoToDraw) {
                    ...
                    doc.addImage(gmailLogoToDraw, gmailFmt, gmailLogoX, 19.8, gmailLogoWidth, 3.8);
                } else {
                    doc.setFillColor(234, 67, 53);
                    doc.rect(gmailLogoX, 19.8, 5.5, 3.8, 'F');
                }

                doc.text(emailStr, 196, 23, { align: 'right' });
                ...
                doc.save(`${pName.replace(/\s+/g, '_')}_BMLT_Exact_Lab_Report.pdf`);
            } catch (err) {
                console.error(err);
        ...
        function renderAdminLedger() {
            ...
            appState.reports.forEach(rep => {
                ...
                tr.className = 'border-b border-slate-800/80 hover:bg-slate-950/40 transition';
                tr.innerHTML = `
                    <td class="p-3 font-mono text-xs text-slate-400">${rep.id}</td>
                    <td class="p-3 font-bold text-cyan-400 font-mono text-xs">${rep.pid}</td>
                    <td class="p-3 text-white font-medium text-xs">${rep.patientName}</td>
                    <td class="p-3 text-cyan-300 font-semibold text-xs">${rep.testTitle || 'Test'}</td>
                    <td class="p-3 text-xs text-slate-400">${rep.date}</td>
                    <td class="p-3 text-right flex justify-end gap-2">
                        <button onclick="downloadExistingReport('${rep.id}')" class="btn-3d-active px-3 py-1 rounded-lg text-xs font-bold transition shadow-sm">
                            1:1 BMLT PDF
                        </button>
                        <button onclick="deleteReport('${rep.id}')" class="bg-red-500/10 text-red-400 border border-red-500/30 px-2 py-1 rounded-lg text-xs hover:bg-red-500 hover:text-white transition">
                            ✕
                        </button>
                    </td>
                `;
